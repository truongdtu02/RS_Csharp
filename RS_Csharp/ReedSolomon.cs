...
namespace FEC
{
    class ReedSolomon
    {
        const int codewordleng = 255;
        const int parleng = 32;
        const double berP = 0.007;
        const int msgleng = (codewordleng - parleng);
        const int looptime = 6;

        const int MAXDEG = (parleng * 2);
        const int MAXDEG2 = (parleng * 4);

        private readonly Random _random = new Random();

        byte[] msg = new byte[msgleng];
        byte[] codeword = new byte[codewordleng + 1];
        byte[] codewordSave = new byte[codewordleng + 1];
        ...
        int[] genPoly = new int[MAXDEG2];

        int DEBUG = 0;

        void GenRand(byte[] _data, int _length)
        {
            int i;
            for (i = 0; i < _length; i++)
            {
                _data[i] = (byte)_random.Next();
            }
        }

        void bsc(byte[] data, double p, int nbyte)
        {
            byte[] bsccode = new byte[nbyte];
            for (int i = 0; i < nbyte; i++)
            {
                bsccode[i] = 0x00;
            }
            int nbits = nbyte * 8;
            int ber = (int)((double)p * nbits);
            for (int i = 0; i < ber; i++)
            {
                int x = _random.Next(nbits - 1);
                bsccode[x / 8] |= (byte)(1 << (x % 8));
            }
            for (int i = 0; i < nbyte; i++)
            {
                data[i] ^= bsccode[i];
        ...
        {
            int i;
            for (i = 0; i < codewordleng; i++)
            {
                codewordSave[i] = codeword[i];
            }
        }
        ...
        void printerrors()
        {
            ...
            for (i = 0; i < codewordleng; i++)
            {
                int diff = codeword[i] ^ codewordSave[i];
                for (j = 0; j < 8; j++)
                {
                    if ((diff & 1) == 1)
                        errorbits++;
            ...
            Console.WriteLine("Error bits: {0}", errorbits);
        }

        public int runReedSolomon()
        {
            ...
            for (int iter = 0; iter < looptime; iter++)
            {
                ...
                encode_data(msg, msgleng, codeword);
                ...
                //byte_err(0x34, 10, codeword);
                //byte_err(0x34, 7, codeword);

                printerrors();

                decode_data(codeword, codewordleng);
        ...
        */
        ...
        void init_exp_table()
        {
            ...
            for (i = 1; i < 256; i++)
            {
                ...
                gexp[i + 255] = gexp[i];
                //if((x < max) && ( x > min)) ~ if(unsingned(x-min) < (max-min))
            }
        ...
        void decode_data(byte[] data, int nbytes)
        {
            ...
            if (check_syndrome() != 0)
            {
                correct_errors_erasures(codeword, codewordleng);
