...
namespace FEC
{
    class ReedSolomon
    {
        ...
        public int decode(byte[] _receivedData, int _lengthReceivedData, byte[] _decodedata) {
            if (_lengthReceivedData > _receivedData.Length) return 0;
            int _lengthDecodeData = _decodedata.Length;

            if (_lengthDecodeData < GetDataLeng(_lengthReceivedData)) return 0;
